...
import com.backroomsoftwarellc.lib.spotlight.target.MultiTarget
...
internal class SpotlightView @JvmOverloads constructor(
  ...
  private val postRenderPaint by lazy { Paint() }
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    if (currentTarget != null && currentShapeAnimator != null) {
      when(target ) {
        is SingleTarget ->
          currentTarget.shape.draw(canvas = canvas, point = currentTarget.anchor, value = currentShapeAnimator.animatedValue as Float, paint = shapePaint)
        is MultiTarget ->
          (currentTarget as MultiTarget).targetPoints.forEach {
            currentTarget.shape.draw(canvas, it, currentShapeAnimator.animatedValue as Float, shapePaint)
          }
      }
    }

    currentTarget?.renderListener?.onPreRender(canvas, currentTarget, postRenderPaint)

    if (currentTarget != null && currentEffectAnimator != null && currentShapeAnimator != null && !currentShapeAnimator.isRunning) {
      when(target ) {
        is SingleTarget ->
          currentTarget.effect.draw(canvas = canvas, point = currentTarget.anchor, value = currentEffectAnimator.animatedValue as Float, paint = effectPaint)
        is MultiTarget ->
          (currentTarget as MultiTarget).targetPoints.forEach {
            currentTarget.shape.draw(canvas, it, currentEffectAnimator.animatedValue as Float, effectPaint)
          }
      }
    }

    currentTarget?.renderListener?.onRendered(canvas, currentTarget, postRenderPaint)
  }
  ...
  fun startTarget(target: Target) {
    ...
    println("Trying to anchor to target " + 1)
    println("Anchor points are " + target.anchor)
